...
async function installAgents(agents, settings = {}) {
  ...
  // Build roster content (reads the written files for frontmatter)
  process.stdout.write(`\n  Generating AGENT_ROSTER.md...`);
  const rosterContent = writeRoster(agents, AGENTS_DIR);
  console.log(chalk.green(' done'));

  // Process templates in all agent files now that we have the roster
  process.stdout.write(`  Processing templates...`);
  for (const agent of agents) {
    const filename = path.basename(agent.file);

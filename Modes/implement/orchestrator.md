...
description: Orchestrates Implement mode. Use when an implementation plan has been approved and is ready to be built. Drives four sequential phases with two gates and a 2-cycle bug loop. All sub-agents are drawn from the team roster at runtime by their modes: [implement] tag.
...
1. **One `<team>-implementation-engineer` per user story** in `IMPLEMENTATION_PLAN.md`.
   - Pass each agent only its own user story section + architecture notes. No other context.
   - Keep context small: one story per agent is intentional — it reduces noise and enables true parallelism.
...
- Only after all report `DONE`: proceed to Phase 2.
...
The project-level pre-tool hook (`check-test-env.sh`) provides a second enforcement layer independent of this conversation — it physically blocks test commands if env vars are not set. See `Skills/shared/environment-validator.md` for setup instructions.
...
1. **Update app context.** Mark `"Update APP_CONTEXT.md"` as `in_progress` in TodoWrite. Invoke `app-context-updater` passing the session path. Use **update** mode if `.aiteam/APP_CONTEXT.md` already exists, **init** mode if this is the first implementation run. For monorepos, let the updater detect structure automatically. Wait for confirmation, then mark `completed`.
